...
export function NavigationListContainer() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        titleKey="page.navigation.title"
        descriptionKey="page.navigation.description"
        action={
          <button
            type="button"
            onClick={() => {
      ...
      {!isLoading && !isError && items.length === 0 ? (
        <EmptyState titleKey="page.navigation.noResults" descriptionKey="common.noResultsHint" />
      ) : null}
      {!isLoading && !isError && items.length > 0 ? (
        <>
          ...
          <DataTable columns={columns}>
            {items.map((item) => (
              <tr key={item.id}>
                ...
                <DataTableCell columnKey="url" className="break-all">
                  {item.url}
                </DataTableCell>
                <DataTableCell columnKey="sort">{item.sort}</DataTableCell>
      ...
      {formOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <form onSubmit={handleSave} className="w-full max-w-md space-y-4 rounded-lg border border-border bg-surface p-6">
            <h2 className="text-lg font-semibold">{editing ? t("common.edit") : t("page.navigation.create")}</h2>
