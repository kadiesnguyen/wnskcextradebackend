...
export function CtMarketListContainer() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && !isError && items.length > 0 ? (<><PageMetaBar meta={meta} isFetching={isFetching} /><DataTable columns={[{ key: "id", label: t("common.id") }, { key: "coinname", label: t("common.coin") }, { key: "title", label: "Title" }, { key: "status", label: t("common.status") }, { key: "actions", label: t("common.actions") }]}>{items.map((item) => (<tr key={item.id}><DataTableCell columnKey="id">{item.id}</DataTableCell><DataTableCell columnKey="id">{item.coinname}</DataTableCell><DataTableCell columnKey="coinname">{item.title}</DataTableCell><DataTableCell columnKey="title">{item.status}</DataTableCell><DataTableCell columnKey="status"><button type="button" onClick={() => { setEditing(item); setCoinname(item.coinname); setTitle(item.title); setFormOpen(true); }} className="text-sm text-primary">{t("common.edit")}</button></DataTableCell></tr>))}</DataTable>{meta ? <PaginationNav meta={meta} onPageChange={(p) => updateParams({ page: String(p) })} isFetching={isFetching} /> : null}</>) : null}
